...
import {
  ...
  type UpsertSchemeCategoryRequest,
...
@Component({
  selector: 'app-scheme-category-form',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './scheme-category-form.html',
  styleUrls: ['../../admin-shared.css', './scheme-category-form.css'],
})
export class SchemeCategoryForm implements OnInit {
  ...
  private async loadExisting(id: number) {
    try {
      ...
      if (!res.success || !res.category) {
        ...
        await this.router.navigate(['/admin/scheme-categories']);
        return;
      }
  ...
  async save() {
    ...
    const req: UpsertSchemeCategoryRequest = {
      slug: f.slug.trim(),
      name: f.name.trim(),
  ...
  cancel() {
    void this.router.navigate(['/admin/scheme-categories']);
  }
}
